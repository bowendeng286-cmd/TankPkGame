...
function useBaseUiCanvas() {
    if (COLS === BASE_COLS && ROWS === BASE_ROWS) return;
...
function startRound() {
    ...
    aiControllers.forEach(({ ctrl }) => {
        ...
        ctrl._repositionTarget = null;
        ctrl._repositionTime = 0;
        ctrl._repositionCooldown = 0;
        ctrl._lastTankPos = null;
        ctrl._lastTargetId = -1;
        ctrl._turnLock = 0;
...
function readHumanInput() {
    for (let i = 0; i < Math.min(config.humanCount, tanks.length); i++) {
        const t = tanks[i];
        t._touchTargetControl = false;
        t._touchMoveStrength = 0;
        ...
        if (input.touchEnabled && i < 2) {
            ...
            if (joystick.active) {
                ...
                t._touchTargetControl = true;
                const moveRange = Math.max(joystick.maxDistance - joystick.deadZone, 1);
                const touchMoveStrength = clamp((joystick.distance - joystick.deadZone) / moveRange, 0, 1);
                t._touchMoveStrength = touchMoveStrength;
                
                // 使用摇杆的死区参数：死区内只旋转不移动
                if (touchMoveStrength <= 0) {
                    // 死区内：只旋转，不移动
...
function updateGame(dt) {
    ...
    for (const t of tanks) {
        ...
        if (t.mouseControl) {
            ...
            const maxTargetOmega = t._touchTargetControl
                ? TANK_TURN_SPEED * TOUCH_TARGET_OMEGA_MULTIPLIER
                : TANK_TURN_SPEED * 3;
            inputOmega = clamp(diff * 8, -maxTargetOmega, maxTargetOmega);
        } else {
            inputOmega = 0;
        ...
        let move = 0;
        if (t.input.forward) move = t._touchTargetControl ? t._touchMoveStrength : 1;
